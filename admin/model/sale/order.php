...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'deliveryman_id'          => $order_query->row['deliveryman_id'],
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, o.firstname, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
    ...
    public function updateOrderDeliveryman($order_id, $data) {

        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET deliveryman_id = " . (int)$data['deliveryman_id'] . " WHERE order_id = " . (int)$order_id);
	...
	public function addOrderHistory($order_id, $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$data['order_status_id'] . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW(), deliveryman = '" . $data['deliveryman'] . "', deliveryman_id = '" . (int)$data['deliveryman_id'] . "'");
	...
	public function getOrderHistories($order_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT oh.deliveryman, oh.date_added, os.name AS status, oh.comment, oh.notify FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '" . (int)$order_id . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
